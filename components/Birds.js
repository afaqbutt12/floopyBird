...
const Birds = ( world, color, position, size ) => {
  const initialBirdShape = Matter.Bodies.rectangle(
    position.x,
    position.y,
    size.width,
    size.height,
  ...
  Matter.World.add(world, initialBirdShape);
  return{
    body: initialBirdShape,
    color,
    position,
    renderer:<BirdShape/>
  };
